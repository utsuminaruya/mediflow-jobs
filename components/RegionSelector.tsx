...
type Props = {
  onChange: (selection: { type: "major" | "region" | null; value: string | null; label?: string }) => void;
  texts: { major_areas: string; regions: string; clear: string };
};
...
export default function RegionSelector({ onChange, texts }: Props) {
  ...
  const handleClick = (type: "major" | "region", value: string, label?: string) => {
    const sel = { type, value, label };
    setSelected(sel);
    onChange(sel);
  };

  const clear = () => {
    const sel = { type: null, value: null, label: undefined };
    setSelected(sel);
    onChange(sel);
  };
    ...
    <div className="card">
      ...
      {tab === "major" ? (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {majorAreas.map(a => (
            <button key={a.id}
              className={`btn ${selected.type==="major" && selected.value===a.id ? "btn-primary" : "btn-ghost"}`}
              onClick={()=>handleClick("major", a.id, a.label)}>
              {a.label}
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {regions.map(r => (
            <button key={r.id}
              className={`btn ${selected.type==="region" && selected.value===r.id ? "btn-primary" : "btn-ghost"}`}
              onClick={()=>handleClick("region", r.id, r.ja)}>
              {r.ja}
            </button>
          ))}
        </div>
